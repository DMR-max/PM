...
bordvakje * koffiebord::druk_af ( ) {
  ...
  while (eerste_rij != nullptr) {
    koffiebord_geheel -> var = eerste_rij;
  ...
  S.zetopstapel(koffiebord_geheel);

  cout << endl << endl;
  //save();

  cout << "koffiebord (van de stapel): " << eerste_rij << endl;
  return eerste_rij;
}

void koffiebord::stapje_terug( ){
  S.haalvanstapel(koffiebord_geheel, breedte, hoogte);
...
/*bordvakje * koffiebord::save ( ){
  stapel S;
  bordvakje * eerste_rij = ingang;
  bordvakje * tweede_rij = eerste_rij -> buren[4];
  S.zetopstapel(buren[4]);
  cout << " in koffiebord: " << buren[4] << endl;
  //printf("&eerste_rij   = %p\n", (void *) &eerste_rij);
  //return eerste_rij;
}*/
